...
describe("<Piano />", () => {
  ...
  describe("noteRange", () => {
    test("requires natural midi numbers", () => {
      const wrapper = mount(
        <Piano
          noteRange={{
            first: MidiNumbers.fromNote("db4"),
    ...
    test("requires note range to be in order", () => {
      ...
      expect(spyConsoleError).toHaveBeenCalledWith(
        expect.stringContaining(
          "noteRange.first must be smaller than noteRange.last"
        )
      );
    });
    test("renders correct keys", () => {
      ...
      expect(wrapper.find(".Piano__Key").length).toBe(13); // Should have 12 + 1 keys
      expect(wrapper.find(".Piano__Key--natural").length).toBe(8); // Should have 7 + 1 natural keys
      expect(wrapper.find(".Piano__Key--accidental").length).toBe(5);
    });
  });
  ...
  describe("playNote and stopNote", () => {
    test("is fired upon mousedown and mouseup", () => {
      ...
      wrapper.find(".Piano__Key").first().simulate("mousedown");

      expect(mockPlayNote).toHaveBeenCalledTimes(1);
      expect(mockStopNote).toHaveBeenCalledTimes(0);

      wrapper.find(".Piano__Key").first().simulate("mouseup");
    ...
    test("is fired upon touchstart and touchend", () => {
      ...
      const firstKey = wrapper.find(".Piano__Key").first();
  ...
  describe("keyboardShortcuts", () => {
    test("key events trigger playNote and stopNote", () => {
      ...
      const keyboardShortcuts = [
        { key: "a", midiNumber: MidiNumbers.fromNote("c3") },
      ];
  ...
  describe("disabled", () => {
    ...
    test("renders disabled key state", () => {
      ...
      expect(wrapper.find(".Piano__Key--disabled").length).toBe(13);
    });
  });
  ...
  describe("className", () => {
    test("adds a className", () => {
      ...
      expect(wrapper.find(".Piano__Keyboard").hasClass("Hello")).toBe(
        true
      );
    });
  });
  ...
  describe("renderNoteLabel", () => {
    test("default value has correct behavior", () => {
      ...
      const firstKey = wrapper.find(".Piano__Key").at(0);
      expect(firstKey.find(".Piano__NoteLabel").text()).toBe("a");
      expect(
        firstKey
          .find(".Piano__NoteLabel")
          .hasClass("Piano__NoteLabel--natural")
      ).toBe(true);

      // Second key should not have label
      const secondKey = wrapper.find(".Piano__Key").at(1);
      expect(secondKey.find(".Piano__NoteLabel").exists()).toBe(false);
    });
    test("works for keys with and without shortcuts", () => {
      ...
      const firstKey = wrapper.find(".Piano__Key").at(0);
      expect(firstKey.find(".label-midiNumber").text()).toBe(
      ...
      const secondKey = wrapper.find(".Piano__Key").at(1);
      expect(secondKey.find(".label-midiNumber").text()).toBe(
        `${MidiNumbers.fromNote("db3")}`
      );
